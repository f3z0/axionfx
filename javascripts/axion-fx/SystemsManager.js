...
define([
	...
	SystemsManager.prototype = {
		...
		renameSystem: function(systemName, newName) {
			for (var i = 0; i < this.systems.length; ++i) {
				var ps = this.systems[i];
				if (ps.systemName === systemName) {
					ps.systemName = newName;
					ps.system.systemName = newName;
					break;
				}
			}
		},
		...
		positionSystems: function(size) {
			for (var i = 0; i < this.systems.length; ++i) {
				var pos = this.systems[i].system.posName;
		...
		addSystem: function(name, actorType, def) {
			this.deleteSystem(name);
			var sys = new System(def);
			sys.systemName = name;
			this.systems.push({'systemName': name, 'actorType': actorType, 'system': sys});
		},
